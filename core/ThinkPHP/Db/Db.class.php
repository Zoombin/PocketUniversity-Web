...
class Db extends Think
{
    ...
    // 查询表达式
    protected $selectSql  =     'SELECT%DISTINCT% %FIELDS% FROM %TABLE%%FORCE%%JOIN%%WHERE%%GROUP%%HAVING%%ORDER%%LIMIT%';
...
//             else
    ...
    public function select($options=array(),$method='',$p='',$p2='') {
        ...
        $sql   = str_replace(
            array('%FORCE%','%TABLE%','%DISTINCT%','%FIELDS%','%JOIN%','%WHERE%','%GROUP%','%HAVING%','%ORDER%','%LIMIT%'),
            array(
                $this->parseForce(isset($options['force'])?$options['force']:''),
                $this->parseTable($options['table']),
    ...
    protected function parseForce($force) {
        return !empty($force)?   ' force index( '.$force.') ':'';
    }
