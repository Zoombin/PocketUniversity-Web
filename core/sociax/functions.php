...
function U($url, $params = false, $redirect = false, $suffix = true)
{
	...
	// 组合默认路径
	$site_url = $siteUrl.'/index.php?'.C('VAR_APP').'='.$app.'&'.C('VAR_MODULE').'='.$mod.'&'.C('VAR_ACTION').'='.$act;
	...
	if (C('URL_ROUTER_ON')) {
		...
		if (isset($router_ruler[$router_key])) {
			//填充路由参数
			$site_url = $siteUrl . '/' . $router_ruler[$router_key];
...
function halt($error) {
    ...
    {
        //否则定向到错误页面
        header("Location: http://pocketuni.net/error.html");die;
...
function forword($act,$mod='',$app='',$jump=false){
    if($act)
        $GLOBALS['ts']['_act']    = $act;
    if($mod)
        $GLOBALS['ts']['_mod']    = $mod;
...
function execAction(){
    //创建Action控制器实例
    $module  =  A($GLOBALS['ts']['_mod'],$GLOBALS['ts']['_app']);
    if(!$module) {
        // 是否存在扩展模块
        $_module = C('_modules_.'.$GLOBALS['ts']['_mod']);
        if($_module) {
            // 'module'=>array('classImportPath'[,'className'])
            import($_module[0]);
            $class = isset($_module[1])?$_module[1]:$GLOBALS['ts']['_mod'].'Action';
            $module = new $class;
        }else{
            // 是否定义Empty模块
            $module = A("Empty");
        ...
        if(!$module)
            // 模块不存在 抛出异常
            throw_exception(L('_MODULE_NOT_EXIST_').$GLOBALS['ts']['_mod']);
    }

    //获取当前操作名
    $action = $GLOBALS['ts']['_act'];
