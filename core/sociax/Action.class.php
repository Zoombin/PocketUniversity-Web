   ...
   /**
	...
	protected function initUser()
	{
		...
		$this->assign('user',$this->user);
                ...
                //lu 超过3天的社团成员申请 设为通过
                if(empty($_SESSION['groupMemberCheck']) || $_SESSION['groupMemberCheck'] <= strtotime('-1 day')){
                    D('Member','group')->allowOver3Day($this->mid);
                    $_SESSION['groupMemberCheck'] = time();
                }
		...
		// 检查是否用户是否验证
		if (!$ts['user']['is_valid'] && $this->mid>0
                        && !(APP_NAME == 'home' && MODULE_NAME=='Public')) {
                    $this->assign('jumpUrl', U('home/Public/userinfo0'));
                    $this->error('请先身份验证');
                    exit;
		}
		// 检查是否用户初始化
		if (!$ts['user']['is_init'] && $this->mid>0
                        && !(APP_NAME == 'home' && MODULE_NAME=='Public')) {
	...
	protected function initUserApp() {
		...
		foreach ($res as $k => &$v) {
                    ...
                    if($v['app_name'] == 'train' && $ts['user']['sid']!=473){
                        unset($res[$k]);
                        continue;
                    }
   ...
   /**
    ...
    protected function checkHash(){
        ...
        if(isset($_SESSION[$name])) {
            // 当前需要令牌验证
            if(empty($_REQUEST[$name]) || $_SESSION[$name] != $_REQUEST[$name]) {
                // 非法提交
                $this->error('验证过期，请刷新页面重新填写');
            }
            // 验证完成销毁session
            unset($_SESSION[$name]);
        }
