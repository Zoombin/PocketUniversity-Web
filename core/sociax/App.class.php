...
{//类定义开始
    ...
    static public function init()
    {
        ...
        //手机支付成功时
        if(MODULE_NAME == 'Account' && ACTION_NAME == 'rechargeList' && $_SESSION['wap_to_normal'] != '1'
                && cookie('wap_to_normal') != '1' && $_REQUEST['wap_to_normal'] != '1' && $_REQUEST['r'] == '1' && isMobile()){
            echo '<center style="padding-top:100px;font-size:100px;color:green;">支付成功！</center>';
            exit;
        }
...
//            	;
//            else if (MODULE_NAME == 'Public' && ACTION_NAME == 'android')
//                ;
//            else if (MODULE_NAME == 'Public' && ACTION_NAME == 'verify')
//                ;
...
//        }
        ...
        // 使用手持设备时, 对home的访问默认跳转至移动版, 除非用户指定访问普通版。
//        if (APP_NAME == 'event' && MODULE_NAME == 'School' && ACTION_NAME == 'index' && $_SESSION['wap_to_normal'] != '1' && cookie('wap_to_normal') != '1' && $_REQUEST['wap_to_normal'] != '1') {
//            $domain = parse_url($_SERVER['SERVER_NAME']);
//            $domain = substr($domain['path'], 0, strpos($domain['path'], '.'));
//            if($domain == 'suda'){
//                if (isiOS() || isAndroid()) // iOS和Android跳转至3G版
...
//        }
    ...
    static private function checkUser() {
        ...
        if (!service('Passport')->isLogged()) { // 未登录
            if ($_SERVER['HTTP_HOST']=='test.pocketuni.net' || $_SERVER['HTTP_HOST']=='test.xyhui.com') {
                redirect("http://www.pocketuni.net/index.php?app=home&mod=Public&act=login");
            }
            // 邀请
...
//                    if(count($arr > 2)){
//                        $schools = M('school')->where("domain != ''")->field('domain')->findAll();
//                        if($schools){
//                            $schools = getSubByKey($schools,'domain');
//                            if(in_array($arr[0], $schools)){
...
//                    }
            ...
            if (!service('SystemPopedom')->hasPopedom()) {
                ...
                elseif(MODULE_NAME=='Puback'||MODULE_NAME=='Admin')
