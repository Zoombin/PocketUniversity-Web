...
class View extends Think
{
    ...
    public function fetch($templateFile='',$charset='',$contentType='text/html',$display=false)
    {
        ...
        $theme_template_path = THEME_PATH.'/apps/'.$GLOBALS['ts']['_app'];
        if(file_exists($theme_template_path.'/'.$GLOBALS['ts']['_mod'].'/'.$templateFile.C('TMPL_TEMPLATE_SUFFIX'))){
                //当前模版路径
                C('TEMPLATE_PATH', $theme_template_path);
                //项目模板目录
                C('APP_TMPL_PATH', THEME_URL.'/apps/'.$GLOBALS['ts']['_app'].'/');
                //项目公共文件目录
                C('APP_PUBLIC_PATH', C('APP_TMPL_PATH').'Public');

                $templateFile	=	$theme_template_path.'/'.$GLOBALS['ts']['_mod'].'/'.$templateFile.C('TMPL_TEMPLATE_SUFFIX');
                C('TMPL_FILE_NAME', C('TEMPLATE_PATH').'/'.$GLOBALS['ts']['_mod'].'/'.$GLOBALS['ts']['_act'].C('TMPL_TEMPLATE_SUFFIX'));
        }else{
            // 自动定位模板文件
    ...
    protected function templateContentReplace($content) {
        ...
        $replace =  array(
            ...
            '__APP__'		=>	PHP_FILE.'?app='.$GLOBALS['ts']['_app'],        // 当前项目地址
            '__URL__'		=>	PHP_FILE.'?app='.$GLOBALS['ts']['_app'].'&mod='.$GLOBALS['ts']['_mod'],        // 当前模块地址
            '__ACTION__'	=>	PHP_FILE.'?app='.$GLOBALS['ts']['_app'].'&mod='.$GLOBALS['ts']['_mod'],     // 当前操作地址
