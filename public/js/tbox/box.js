    ...
    init:function(option){
		if( !this.inited ){
			$('body').prepend( this.WRAPPER );
                ...
                option = jQuery.extend({}, ui.box.DEFAULTS, option || {});
		if(option.title){
                    var cc = "<h2>"+option.title+"</h2>";
                    if (option.closeable) {
                        cc += "<a href='#' class='close'></a>";
                    }
                    $('#tsbox').find('.title-bar').html(cc);

                }
		if(option.zIndex)
			$('#tsbox').css('z-Index',option.zIndex);
