$.extend({
    weibo:function(setting){
        ...
        $.extend(this,{
                ...
                //点赞
                heart:function(id,o,num){
                    operateFactory.create('heart',function(txt){
                        if( txt ){
                            $(o).wrap('<span id=content_'+id+'></span>');
                            newNum = num+1;
                            $('#content_'+id).html('赞('+newNum+')');
                        }else{
                            ui.error('点赞失败');
                        }
                    },{id:id});
        ...
        var textareaStatus = function(type,obj){
            ...
            switch(type){
                case 'on':
                    obj.removeAttr('disabled').removeClass('btn_big_disable').addClass('btn_big_after1');
                break;
                case 'off':
                    obj.attr('disabled','true');
                break;
                case 'sending':
                    obj.attr('disabled','true').removeClass('btn_big_after1').addClass('btn_big_disable');
                break;
            }
        },
