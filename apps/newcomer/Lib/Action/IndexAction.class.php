...
class IndexAction extends BaseAction{
	...
	public function news(){
		...
		$document	=	$documentDao->where($map)->order($order)->limit(1)->find();
		if(!$document){
		...
		$category = $document['category0'];
		$documents	=	$this->getDocList($category, 0, 20);
	...
	public function getDocList($category, $isrecom, $limit) {
		$order	=	'mtime DESC, document_id DESC';
		$map['is_active']	=	1;
		...
		if(is_array($category)) {
			$map['category0']	=	array("in", $category);
		} else if($category>0) {
			$map['category0']	=	$category;
		}
		$documents	=	D('Document')->order($order)->where($map)->findPage($limit);
		return $documents;
	}

	public function index(){
		$documents	=	$this->getDocList(array(1,2), 1, 7);
		$this->assign('documents',$documents);
		$photos	=	$this->getDocList(3, 1, 4);
		$this->assign('photos',$photos);
