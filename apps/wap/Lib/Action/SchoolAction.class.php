...
class SchoolAction extends SchoolBaseAction {
    ...
    public function kc() {
        ...
        if ($searchTitle) {
            if (mb_strlen($searchTitle, 'utf8') < 2) {
                $this->redirect(U('wap/School/kc'), 3, '至少输入两个字');
            }
    ...
    Private function _getCourse($id) {
        ...
        if ($result) {
            if ($result['sid'] != $this->sid) {
                $this->redirect(U('wap/School/kc'), 3, '课程不存在');
            }
            if ($result['status'] == 1) {
                $this->redirect(U('wap/School/kc'), 3, '课程正在审核中');
            } elseif ($result['status'] == 0) {
                $this->redirect(U('wap/School/kc'), 3, '课程未通过审核');
            }
            $this->assign($result);
        } else {
            $this->redirect(U('wap/School/kc'), 3, '课程不存在或，未通过审核或，已删除');
        }
        return $result;
    }
    ...
    Private function _getHd($id) {
        ...
        if ($result) {
            if ($result['sid'] != $this->sid) {
                $this->redirect(U('wap/School/hd'), 3, '课程活动不存在');
            }
            if ($result['status'] == 1) {
                $this->redirect(U('wap/School/hd'), 3, '课程活动正在审核中');
            } elseif ($result['status'] == 0) {
                $this->redirect(U('wap/School/hd'), 3, '课程活动未通过审核');
            }
            $this->assign($result);
        } else {
            $this->redirect(U('wap/School/hd'), 3, '课程活动不存在或，未通过审核或，已删除');
        }
        return $result;
    }
    ...
    public function join() {
        $id = intval($_REQUEST['id']);
        if(!$id){
            $this->redirect(U('wap/School/kc'), 3, '参数错误');
        }
        $kcUrl = U('wap/School/kcDetail',array('id'=>$id));
        if (!$this->smid) {
    ...
    public function joinHd() {
        $id = intval($_REQUEST['id']);
        if(!$id){
            $this->redirect(U('wap/School/hd'), 3, '参数错误');
        }
        $kcUrl = U('wap/School/hdDetail',array('id'=>$id));
        if (!$this->smid) {
            $this->redirect($kcUrl, 3, '您不是该校用户');
        }
