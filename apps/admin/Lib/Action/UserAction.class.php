...
//        $login = M('login_record')->group('uid')->field('uid, max(ctime) as ctime')->findAll();
//        $dao = M('login_count');
//        foreach ($login as $value) {
//            $dao->add($value);
//        }
//        $mobile = M('login')->group('uid')->field('uid')->findAll();
...
//            $daoE = M('event');
//            $daoEU = M('event_user');
//            $daoEP = M('event_player');
//            $daoEI = M('event_img');
//            $daoEF = M('event_flash');
//            $daoEV = M('event_vote');
//            $user = $daoEU->where('uid=0')->findAll();
//            foreach($user as $v){
//                $pid = $v['id'];
//                unset($v['id']);
//                unset($v['uid']);
...
//                    // 修改靓照，视频，投票者数据
//                    var_dump($newPid);
//                    $daoEI->setField('uid', $newPid, 'uid='.$pid);
//                    $daoEF->setField('uid', $newPid, 'uid='.$pid);
//                    $daoEV->setField('pid', $newPid, 'pid='.$pid);
//                    //删除选手，修改参加者人数
//                    $daoEU->where('id='.$pid)->delete();
//                    $daoE->setDec('joinCount', 'id='.$v['eventId']);
//                    $daoE->setInc('limitCount', 'id='.$v['eventId']);
//                }
//            }
//            die;
//        }
...
//            $daoEF = M('event_flash');
//            $daoEV = M('event_vote');
//            $user = $daoEU->where('ticket>0')->findAll();
//            foreach($user as $v){
//                $pid = $v['id'];
//                unset($v['id']);
//                unset($v['uid']);
...
//            set_time_limit(0);
//            $daoSchool = M('school');
//            $map['title'] = array('exp', 'REGEXP "^.*[级]$"');
//            $sids = $daoSchool->field('id,title')->where($map)->findAll();
//            $dao = M('user');
//            foreach ($sids as $vo) {
//                $data = array();
//                $data['year'] = $vo['title'];
//                $dao->where('sid2=' . $vo['id'])->save($data);
//                $daoSchool->where('id=' . $vo['id'])->delete();
//            }
//            var_dump($sids);
//            die;
//        }
...
//            $daoSchool = M('school');
//            $dao = M('user');
//            $ids = array('0', '1', '2', '3', '4', '472', '402', '61', '62', '283', '393');
//            $map['pid'] = array('not in', $ids);
//            $sids = $daoSchool->field('id,pid,title')->where($map)->findAll();
//            //var_dump($sids);
//            foreach ($sids as $vo) {
//                //$data[] = preg_replace('/[0-9-\n\r]/i','',$vo['title']);
//                $data['major'] = preg_replace('/[0-9-\n\r]/i', '', $vo['title']);
//                $dao->where('sid3=' . $vo['id'])->save($data);
//                $daoSchool->where('id=' . $vo['id'])->delete();
//            }
//            var_dump($sids);
//            die;
...
//    }
    ...
    public function schoolEvent() {
        $dao = model('Schools');
        $list = $dao->where(array('pid' => 0))->order('display_order ASC')->findAll();
    ...
    public function doChangeCity(){
        ...
        $data['cityId'] = intval($_REQUEST['cityId']);
...
//        }
    ...
    public function doChangeRegister() {
        $dao = M('school');
        $map['id'] = intval($_REQUEST['id']);
        $type = ($_REQUEST['type'] == 'open') ? 1 : 0;
        $result = $dao->where($map)->setField('canRegister', $type);
        if ($result) {
            echo 1;
            exit;
        }
    ...
    public function renameSchool() {
        ...
        $res = model('Schools')->save($data);
        if (false !== $res) {
            $school = model('Schools')->field('id,pid')->where('id=' . $id)->find();
            if($school['pid']==0){
...
//        $this->display();
   ...
   public function childTree() {
    ...
    public function doEditUser() {
        ...
        if (mb_strlen($_POST['major'], 'UTF8') > 10) {
            $this->error('专业格式不正确');
        }
        //保存修改
...
EOD;
    ...
    public function doSearchUser() {
        ...
        //模糊查询
        $fields2 = array('uname', 'email','year','major','mobile');
