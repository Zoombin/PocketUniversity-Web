...
/*
		...
		if($school==0) {
			$school = intval($_SESSION['doc_school']);
		} else {
			$_SESSION['doc_school'] = $school;
		}
		if($school<=0 || $school>count($schools)) {
			$school = 0;
		} else {
			$school--;
		}
		...
		$this->assign('school',$schools[$school]['t']);
		$this->assign('schools',$schools);
*/
                //unset($_SESSION['doc_school']);
                $schools = model('Schools')->_makeTree(0);
                $sCat = model('Schools')->__getCategory();
...
//                }
	...
	public function getSchool(){
            return model('Schools')->getChildIds($this->school);
	}
