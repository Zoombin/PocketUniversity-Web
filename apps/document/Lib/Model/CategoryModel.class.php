...
{
	var $tableName = 'wenku_category';
	//生成分类Tree
	public function _makeTree($pid) {
		
		if($pid==0 && $cache = S('Cache_Document_Cate_'.$pid)){ // pid=0 才缓存
			return $cache;	
		}
		...
		$pid==0 && S('Cache_Document_Cate_'.$pid,$cTrees);	// pid=0 才缓存
		return $cTrees;
	}
	...
	public function _makeTopTree() {
		...
		$pid = 0;
		if($pid==0 && $cache = S('Cache_Document_Cate_top_'.$pid)){ // pid=0 才缓存
			return $cache;	
		}	
		...
		$pid==0 && S('Cache_Document_Cate_top_'.$pid,$cTrees);	// pid=0 才缓存
		return $cTrees;
	}	
	//获取LI列表
