<?php
    ...
    class CommentAction extends Action{
        ...
        public function doAddComment() {
        	...
        	$_POST['url']					= str_replace('xyhui.com','pocketuni.net',urldecode($_POST['url']));
        	...
        	if ($res) {
        		...
        		if ($_POST['with_new_weibo']) {
                            ...
                            $from_data = serialize($from_data);
                            D('Weibo', 'weibo')->publish($this->mid, array(
                                'content' => html_entity_decode(
                                        $_POST['comment'] . ($_POST['to_id'] > 0 ? (' //@' . getUserName($former_comment['uid']) . ' :' . $former_comment['comment']) : ''), ENT_QUOTES
                                ),
                                    ), 0, 0, '', '', $from_data);
                        }
/*
