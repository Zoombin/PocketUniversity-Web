...
class SpaceAction extends Action {
            ...
            if ($user_info) {
                if($user_info['sid']=473){
                    ...
                    $userinfo = array(
                        '微博地址' => U('home/Space/index', array('uid' => $user_info['domain'] ? $user_info['domain'] : $this->uid)),
                        '性别' => getSex($user_info['sex']),
                        '所在学校' => tsGetSchoolName($user_info['sid']),
                    );
                }
                // 基本信息-钩子
    ...
    public function index() {
        ...
        if (in_array($data['type'], array('', 'original', '1', '3', '4'))) {
            $data['list'] = D('Operate', 'weibo')->getSpaceList($this->uid, $data['type'],$this->mid);
        }
    ...
    function follow() {
        ...
        // 关注的人列表
        $data['list'] = D('Follow', 'weibo')->getList($this->uid, $data['type'], 0, $data['gid'],5);
        $this->assign($data);
