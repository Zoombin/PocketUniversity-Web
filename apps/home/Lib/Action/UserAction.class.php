...
class UserAction extends Action {
    ...
    function index() {
        ...
        //} else {
            ...
            $order = 'weibo_id DESC';
            $guanzu = D('Operate','weibo')->doSearchTopic("",$order,$this->mid);
            $this->assign ('guanzu', $guanzu );
            //var_dump($guanzu);die;
        //}
       ...
       $this->__assignTabSwitch();
        ...
        $announces = M('announce')->where($map)->field('id,title')->order('id DESC')->limit(3)->findAll();
        $this->assign('announces',$announces);
        $this->assign('adhome',getAd($this->user['sid'],substr($this->user['year'],0,2),2,5));
        $this->assign('adleft',getAd($this->user['sid'],substr($this->user['year'],0,2),3,1));
        $this->display ('index1305');
    }

    public function adClick(){
        $adid = intval($_REQUEST['adid']);
        if($adid<0)return false;
        D('Ad','admin')->adClick($adid,$this->mid);
    }
	/**
     *
     */
        ...
        switch ($data ['type']) {
            ...
            case 'newjoin' :
                $data ['list'] = M ("user")->where("a.is_active=1 AND a.is_init=1 AND a.uid!={$this->mid} AND a.uid not in (SELECT fid FROM ".C('DB_PREFIX')."weibo_follow as b WHERE b.uid={$this->mid}) ")
											->field('a.uid,a.uname,a.domain,a.ctime')
