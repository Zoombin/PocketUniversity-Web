...
class CategoryModel extends Model {
    ...
    var $tableName = 'coupon_category';

    //生成分类Tree
    public function _makeTree($pid) {
        if ($pid == 0 && $cache = S('Cache_Coupon_Cate_' . $pid)) { // pid=0 才缓存
            return $cache;
        }
        ...
        $pid == 0 && S('Cache_Coupon_Cate_' . $pid, $cTrees); // pid=0 才缓存
        return $cTrees;
    }
    ...
    public function _makeTopTree() {
        ...
        $pid = 0;
        if ($pid == 0 && $cache = S('Cache_Coupon_Cate_top_' . $pid)) { // pid=0 才缓存
            return $cache;
        }
        ...
        $pid == 0 && S('Cache_Coupon_Cate_top_' . $pid, $cTrees); // pid=0 才缓存
        return $cTrees;
    }
