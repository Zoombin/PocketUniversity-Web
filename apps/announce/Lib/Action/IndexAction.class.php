...
class IndexAction extends Action {
    ...
    public function _initialize() {
        ...
        $schools = model('Schools')->__getCategory(0);
        if($_SESSION['announce_sid'] <= 0 || !array_key_exists($_SESSION['announce_sid'], $schools)){
            foreach ($schools as $key => $value) {
                $_SESSION['announce_sid'] = $key;
                break;
            }
        }
        $this->assign('school', $schools[$_SESSION['announce_sid']]['title']);
