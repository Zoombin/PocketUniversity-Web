...
function get_image_url($savepath) {
	if(strlen(trim($savepath))>0) {
			...
			return '<img src="'.SITE_URL . '/data/uploads/' . $savepath.'" style="width:25px;height:25px" valign="middle"/>';
...
function get_image($savepath, $url) {
	if(strlen(trim($savepath))>0) {
		if(strlen(trim($url))>0) {
			return '<a href="'.$url.'"><img src="'.SITE_URL . '/data/uploads/' . $savepath.'" /></a>';
		} else {
			return '<img src="'.SITE_URL . '/data/uploads/' . $savepath.'" />';
		}
	}
	return "";
}
...
function get_link($savepath, $url, $style="") {
	if(strlen(trim($savepath))>0) {
		if(strlen(trim($url))>0) {
			return '<a href="'.$url.'" target="_blank"><img src="'.SITE_URL . '/data/uploads/' . $savepath.'" '.$style.'/><i></i></a>';
		} else {
			return '<a href="javascript:void(0)"><img src="'.SITE_URL . '/data/uploads/' . $savepath.'" '.$style.'/><i></i></a>';
		}
	}
	return "";
}
