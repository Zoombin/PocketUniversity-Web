...
{
	var $tableName = 'appstore_category';
	//生成分类Tree
	public function _makeTree($pid, $type) {
		
		if($pid==0 && $cache = S('Cache_Appstore_Cate_'.$pid.'_'.$type)){ // pid=0 才缓存
			return $cache;	
		}	
			
		if( $c = $this->where("pid='$pid' AND type='$type'")->order( 'display_order ASC, id ASC' )->findAll() ){
			if ($pid == 0) {
				foreach($c as $v){
					$cTree['t']	=	$v['title'];
					$cTree['a']	=	$v['id'];
					$cTree['d']	=	$this->_makeTree($v['id'], $type);
					$cTrees[]	=	$cTree;
				}
			} else {
				foreach($c as $v){
		...
		$pid==0 && S('Cache_Appstore_Cate_'.$pid.'_'.$type,$cTrees);	// pid=0 才缓存
		return $cTrees;
	}
	//获取LI列表
