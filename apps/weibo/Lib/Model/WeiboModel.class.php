...
class WeiboModel extends Model{
        ...
        if( $id = $this->add( $save ) ){
		    ...
		    if(!empty($userLoginInfo)) {
		    	...
		    	S('S_userInfo_'.$uid, $userLoginInfo);
                    ...
                    Model('UserCount')->addCount($uid,'weibo');
    ...
    function notifyToAtme($uid,$id,$content,$transpond_uid,$addCount=true){
   	...
   	function getOne($id,$value,$api=false,$uid=0){
   		if($api){
   			return $this->getOneApi($id,$value,$uid);
   		}else{
    ...
    public function getOneLocation($id, $value, $show_transpond = true,$uid)
    {
        ...
        $result['is_hearted'] = isset($value['is_hearted']) ? intval($value['is_hearted']) : D('Heart','weibo')->isHearted($value['weibo_id'], $uid);
    ...
    public function getOneApi($id, $value, $uid = 0) {
        ...
        if ($value['type'] == 3 && $value['transpond_id'] == 0) {
            $value['type_data'] = null;
        }
        ...
        $value['favorited'] = isset($value['favorited']) ? intval($value['favorited']) : (int)D('Favorite','weibo')->isFavorited($id, $uid);
        $value['is_hearted'] = isset($value['is_hearted']) ? intval($value['is_hearted']) : (int)D('Heart','weibo')->isHearted($id, $uid);
        return $value;
    }

    public function __parseTemplate( $value ){
    	static $rand;
    	if ($rand) {
    		$rand++;
    	}else {
