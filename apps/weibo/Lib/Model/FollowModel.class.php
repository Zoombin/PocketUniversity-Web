...
class FollowModel extends Model {
    ...
    public function getNowFollowing($uid) {
        if ($uid == 1) {
            return array();
        }
        $list = $this->where('uid=' . $uid . ' AND type=0')->field('fid')->findAll();
        if ($list) {
            return getSubByKey($list, 'fid');
        }
        return array();
    }
    ...
    public function getUserFollowCount($uid) {
        ...
        if (empty($followCount) && $followCount !== '0') {
            $followCount = Model('UserCount')->getUserFollowingCount($uid);;
        }
    ...
    public function dofollow($mid, $fid, $type = 0) {
        ...
        if (0 == $this->where($map)->count()) { // 未关注
            ...
            if ($type == 0) { // 关注用
                ...
                Model('UserCount')->addCount($mid,'following');
                Model('UserCount')->addCount($fid,'follower');
                // 通知和动态
    ...
    public function unfollow($mid, $fid, $type = 0) {
        ...
        if ($this->where($map)->delete()) { // 取消成功
            if ($map['type'] == 0) { // 取消关注用户时
                ...
                Model('UserCount')->decCount($mid,'following');
                Model('UserCount')->decCount($fid,'follower');
//
//                // 关注记录 - 漫游使用
//                unset($map);
//                $map['uid'] = $mid;
...
//					$list = $this->where("uid={$uid} AND type={$type} ")->order('follow_id DESC')->findpage($limit);
         ...
         */
        //用户地址
        $userInfo = M('User')->where("uid in (" . implode(",", $fids) . ")")->field('realname,sid,uid')->findAll();
        ...
        foreach ($userInfo as $v) {
            $fidHash[$v['uid']]['user']['realname'] = $v['realname'];
            $fidHash[$v['uid']]['user']['sid'] = $v['sid'];
        }
//		foreach ($following as $v){
//			$fidHash[$v['uid']]['following'] =$v['count'];
//		}
        foreach ($fids as $v) {
            $userCount = Model('UserCount')->getUserCount($v);
            $fidHash[$v]['following'] = $userCount['following'];
            $fidHash[$v]['follower'] = $userCount['follower'];
        }
...
//		}
    ...
    public function getStateByArr($uid, $followArr) {
        ...
        foreach($followArr as $v){
            if($this->where('uid='.$uid.' and fid='.$v)->field('uid')->find()){
                $data[$v] = 1;
            }else{
                $data[$v] = 0;
            }
...
//            $sql = "select count(1) as count from (select uid from ts_user where uname LIKE '{$key}%' union select uid from ts_user where realname LIKE '{$key}%') as tt";
//            $countArr = $this->query($sql);
//            $count = $countArr[0]['count'];
//            $sql = "SELECT * from ts_user where uname LIKE '{$key}%' union select * from ts_user where realname LIKE '{$key}%'";
//            $list = $this->findPageBySql($sql, $count);
            $map['realname'] = array('like',$key.'%');
            $list = M('user')->where($map)->findPage(10);
            /*
             * 缓存用户的基本信息, 粉丝数, 关注数
             */
