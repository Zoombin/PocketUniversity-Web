...
//	    		preg_match_all('/#(.*)#/isU',$info['content'],$topic_arr);
			    ...
			    if(!empty($userLoginInfo)) {
			    	...
			    	S('S_userInfo_'.$uid, $userLoginInfo);
                            ...
                            Model('UserCount')->decCount($uid,'weibo');
    ...
    //搜索话题--TODO
    public function doSearchWithTopic($key, $type,$uid=0) {
    	global $ts;
    	//$key = addslashes(t($key));
    	if (!$key) {
    		$list['count'] = 0;
    	...
    	foreach($list['data'] as $key=>$value){
    		$list['data'][$key] = $this->getOne('',$value,false,$uid);
    	}
    	return $list;
    }
        ...
        public function apiDoSearchWithTopic($key,$limit=10,$page,$listMap,$uid) {
        if (!$key) {
            $list['count'] = 0;
            return $list;
        }
        ...
        $topicId = D('Topic', 'weibo')->getTopicId($key);
        $weiboTopicLinkModel = M('weibo_topic_link');
        $list = $weiboTopicLinkModel->where("topic_id = {$topicId}")->order('weibo_id DESC')->select();
        $weiboIdArr = getSubByKey($list, 'weibo_id');
        $listMap['weibo_id'] = array('IN', $weiboIdArr);
        $listMap['isdel'] = 0;
        $offset = ($page - 1) * $limit;
        $data = $this->field('weibo_id')->where($listMap)->order('weibo_id DESC')->limit("$offset,$limit")->findAll();
        $weibo_id_list = getSubByKey($data, 'weibo_id');
        $list = $this->getWeiboDetail($weibo_id_list);
        /*
         * 缓存用户信息, 被转发微博的详情
         */
        $ids = getSubBeKeyArray($list, 'transpond_id,uid');
        $transpond_list = $this->getWeiboDetail($ids['transpond_id']);
        // 本页的用户IDs = 作者IDs + 被转发微博的作者IDs
        $ids['uid'] = array_merge($ids['uid'], getSubByKey($transpond_list, 'uid'));
        D('User', 'home')->setUserObjectCache($ids['uid']);
        foreach($list as $k=>$v){
          $v['favorited'] = D('Favorite','weibo')->isFavorited($v['weibo_id']);
	...
	function doSearchTopic($map,$order,$uid,$limit = true) {
		...
		foreach($list['data'] as $key=>$value){
			...
			$value['is_hearted'] = D('Heart','weibo')->isHearted($value['weibo_id'], $uid, $weibo_ids);
			$list['data'][$key] = $this->getOne('', $value);
		}
		return $list;
	}
    ...
    private function __optToNew($uid, $lastId, $type = null, $follow_gid = null, $limit = 0, $since = false) {
        ...
        if ($since) {
            if ($lastId > 0) {
                $map['weibo_id'] = array('lt', $lastId);
                //$weiboId = "weibo_id<{$lastId} and";
            }
        } else {
            if ($lastId > 0) {
                $map['weibo_id'] = array('gt', $lastId);
                //$weiboId = "weibo_id>{$lastId} and";
            }
        }
        ...
        if ($followCount) {
            if (!empty($follow_gid)) {
                ...
                $map['uid'] = array('in', $fids);
                //$map.=' AND uid IN (' . implode(',', $fids) . ')';
            } elseif($uid!=1) {
                $uidArr = M('weibo_follow')->where('uid='.$uid.' and type=0')->field('fid')->findAll();
        ...
        if ($limit) {
            ...
            foreach ($data as $key => $value) {
                $result[] = $this->getOne('', $value,false,$uid);
            }
            $list['data'] = $result;
        } else {
...
//	}
    ...
    public function getSpaceList($uid, $type, $mid) {
    	if ($type == 'original') { // 原创
        ...
        return $this->_paramResultData($list,$mid,true);
    }

    public function getAdminSpaceList($map) {
        $list = $this->field('weibo_id')->where($map)->order('weibo_id DESC')->findPage(20);
        return $this->_paramResultData($list,0,true);
    }
...
//    				 ->findPage(10, $count);
    	...
    	foreach ($list['data'] as $key => $value) {
        	...
        	$value['is_hearted'] = D('Heart','weibo')->isHearted($value['weibo_id'], $uid, $ids['weibo_id']);
                $list['data'][$key] = $this->getOneLocation('',$value,true,$uid);
        }
        return $list;
    }
    ...
    function getCollection($uid,$api){
    	...
    	*/
    	...
    	$list = M('weibo_heart')->where("`uid`='{$uid}'")->order('`weibo_id` DESC')->findPage();
    ...
    private function _paramResultData($list,$uid,$page=false){
        ...
        */
        ...
        foreach( $data as $key=>$value){
            ...
            $data[$key] = $this->getOne('',$value,false,$uid);
        }
        if($page){
            $list['data'] = $data;
        }else{
