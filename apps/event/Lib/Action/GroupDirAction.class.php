<?php
class GroupDirAction extends GroupBaseAction
{
	var $dir;
	...
	function index() {
		...
		$this->assign('fileList', $fileList);
                $this->setTitle("共享 - " . $this->groupinfo['name']);
		$this->display();
	}
		...
		if(isset($_POST['uploadsubmit']) || $_GET['ajax'] == 1) {
        	...
        	if($info['status']){  //上传成功
            	...
            	if($result) {
        			...
        			$this->redirect('event/GroupDir/upload', array('gid'=>$this->gid,'fid'=>$result));
            	}else{
			        if ($_GET['ajax'] == 1) {
