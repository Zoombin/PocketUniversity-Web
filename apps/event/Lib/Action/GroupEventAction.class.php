...
class GroupEventAction extends GroupBaseAction {
    ...
    public function _initialize() {
        ...
        $this->setTitle("大事记 - " . $this->groupinfo['name']);
    }
    ...
    public function Index() {
        ...
        $topiclist = $this->topic
                ->order('addtime DESC')
                ->where('is_del=0  AND isEvent=1 AND isRule=0 AND gid=' . $this->gid . $search_key)
                ->findPage(10);
//        var_dump($this->topic->getLastSql());die;
    ...
    // 发表话题 编辑话题
    public function add() {
        $this->setTitle("发布新大事记 - " . $this->groupinfo['name']);
        $this->display();
    }
    ...
    public function doAdd() {
        ...
        if ($tid) {
            ...
            $this->assign('jumpUrl', U('/GroupEvent/index', array('gid' => $this->gid, 'tid' => $tid)));
            $this->success('发布大事记成功');
        } else {
            $this->error('发布大事记失败');
        }
    }
    ...
    public function edit() {
        ...
        if (isset($_POST['editsubmit']) && trim($_POST['editsubmit']) == 'do') {
            ...
            $this->assign('jumpUrl', U('event/GroupEvent/event', array('gid' => $this->gid, 'tid' => $tid)));
            $this->success('编辑大事记成功');
        }

        $this->assign('thread', $thread);
        $this->setTitle("编辑大事记 - " . $this->groupinfo['name']);
        $this->display();
    }
    ...
    public function event() {
        ...
        $thread = $this->topic->getEvent($tid);
        if (!$thread)
            $this->error('大事记不存在');
        // 附件信息
        if ($thread['attach']) {
        ...
        $this->setTitle("{$thread['title']} - 大事记 - {$this->groupinfo['name']}");
    ...
    public function del() {
        ...
        if ($_POST['type'] == 'thread') {
            ...
            if ($_POST['ajax'] == 1) {
                ...
                $this->redirect('event/GroupEvent/index', array('gid' => $this->gid));
            }
        }
    }
    ...
    protected function _setOperationLog($operation, &$post_info) {
        ...
        foreach ($post_info as $v) {
            ...
            $content = '把 ' . getUserSpace($v['uid'], 'fn', '_blank', '@' . getUserName($v['uid']))
                    . ' 的大事记“<a href="' . U('event/GroupEvent/event', array('gid' => $this->gid, 'tid' => $v['id'])) . '" target="_blank">'
                    . $v['title'] . '</a>” ' . $operation;
  ...
  public function groupEvent() {
        ...
        $search_key = $search_key ? " AND title LIKE '%{$search_key}%' " : '';
        $list = M('event')
                        ->where('isDel=0 AND status=1 AND is_school_event=' . $this->sid . ' AND gid=' . $this->gid . $search_key)
                        ->field('id,title,joinCount,typeId,cTime')->findPage(20);
        foreach ($list['data'] as $k => $v) {
            $list['data'][$k]{'type'} = M('event_type')->where('id =' . $v['typeId'])->getField('name');
        }
        $res = M('event_group')->where('uid=' . $this->mid . ' AND  gid =' . $this->gid)->find();
        if ($res) {
            $this->assign('can_add_event', 1);
        }
        $this->assign($list);
           $this->setTitle("部落活动 - " . $this->groupinfo['name']);
        $this->display();
    }
