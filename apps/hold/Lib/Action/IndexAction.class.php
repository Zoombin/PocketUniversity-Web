...
class IndexAction extends BaseAction {
    ...
    public function index() {
        $this->assign("menu", 1);
        $list = M('hold_info')->where(array('isDel'=>0))->order('id DESC')->limit(6)->findAll();
        $this->assign("news", $list);
        $list = M('hold_user')->where(array('isDel'=>0))->order('ticket DESC')->limit(12)->findAll();
        $this->assign("player", $list);
        $list = M('hold_user_img')->where(array('uid'=>0))->order('id DESC')->limit(8)->findAll();
        $this->assign("photo", $list);
        //$list = M('hold_user_flash')->where(array('uid'=>0))->order('id DESC')->limit(6)->findAll();
        $list = M('hold_user_flash')->order('id DESC')->limit(6)->findAll();
        $this->assign("video", $list);
        $photoOrder = M('hold_user_img')->where(array('uid' => 0))->order('id DESC')->field('id')->findAll();
    ...
    public function video() {
        $this->assign("menu", 4);
        $list = M('hold_user_flash')->order('id DESC')->findPage(12);
        $this->assign('url', U('hold/index/video'));
        $this->assign($list);
        $this->display();
    }
    ...
    public function photo() {
        $this->assign("menu", 5);
        $dao = M('hold_user_img');
        $list = $dao->where(array('uid' => 0))->order('id DESC')->findPage(9);
        $this->assign('url', U('hold/index/photo'));
        $this->assign($list);
        $photoOrder = $dao->where(array('uid' => 0))->order('id DESC')->field('id')->findAll();
        $photoOrder = array_flip(getSubByKey($photoOrder, 'id'));
        $this->assign('photoOrder',$photoOrder);
        $this->display();
    }
    ...
    public function playFlash(){
        ...
        $app = M('hold_user_flash')->where("`id`={$id}")->find();
        if (!$app) {
    ...
    public function jsonPhoto(){
        $dao = M('hold_user_img');
        $result = array();
        $result['slide']['title'] = 'hold住我的麦克风-照片';
        ...
        $result['next_album']['title'] = 'hold我的麦克风-照片';
    ...
    public function jsonPhoto2(){
        $id = intval($_GET['id']);
        $dao = M('hold_user_img');
        $obj = M('hold_user')->where("`id`={$id}")->find();
        if (!$obj) {
