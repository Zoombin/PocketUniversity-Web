...
class PublicAction extends Action {
	...
	public function doLogin() {
		...
		if (empty($_POST['sid'])) {
			$this->redirect(U('w3g/Public/login'), 3, '请选择学校');
		}
		if (empty($_POST['number']) || empty($_POST['password'])) {
			$this->redirect(U('w3g/Public/login'), 3, '学号和密码不能为空');
		}
	...
	{
            if (empty($_POST['sid']))
                $this->redirect(U('/Public/register', $_POST), 3, '请选择学校');
            if (empty($_POST['number']))
                $this->redirect(U('/Public/register', $_POST), 3, '请填写学号');
            if (empty($_POST['uname']))
                $this->redirect(U('/Public/register', $_POST), 3, '请填写昵称');
