...
class AdminAction extends HoutaiAction {
    ...
    public function _initialize() {
        parent::_initialize();
        $this->info = M('show_info');
        $this->user = M('show_user');
        $this->img = M('show_user_img');
        $this->flash = M('show_user_flash');
    }
    ...
    public function index() {
        $order = 'id DESC';
        $map['isDel'] = 0;
        $list = M('show_info')->where($map)->order($order)->findPage(10);
        $this->assign($list);
        $this->display();
    }
    ...
    public function userList() {
        ...
        $order = 'id DESC';
        $map['isDel'] = 0;
        $list = M()->table("{$db_prefix}show_user AS a LEFT JOIN {$db_prefix}show_user_img AS b ON a.id=b.uid LEFT JOIN {$db_prefix}show_user_flash AS c ON a.id=c.uid")
                        ->field('a.* , GROUP_CONCAT(DISTINCT b.id) AS img, GROUP_CONCAT(DISTINCT c.id) AS flash')->group('a.id')->where($map)->order($order)->findPage(10);
        $this->assign($list);
        $this->display();
    }
    ...
    public function imgList() {
        ...
        $this->assign("showUser", $user);
        $map['uid'] = $id;
        $list = M('show_user_img')->where($map)->order('`id` DESC')->findAll();
    ...
    public function flashList() {
        ...
        $this->assign("showUser", $user);
        $map['uid'] = $id;
        $list = M('show_user_flash')->where($map)->order('`id` DESC')->findAll();
        $this->assign('data', $list);
        $this->assign('uid', $id);
        $this->display();
    }
    ...
    public function videoList() {
        $list = M('show_user_flash')->where(array('uid' => 0))->order('`id` DESC')->findPage(10);
        $this->assign($list);
        $this->display();
    }
    ...
    public function photoList() {
        $list = M('show_user_img')->where(array('uid' => 0))->order('`id` DESC')->findPage(10);
        $this->assign($list);
        $this->display();
    }
    ...
    protected function _upload() {
        ...
        $options['save_path'] = UPLOAD_PATH . '/show/';
        $options['save_to_db'] = false;
        //$saveName && $options['save_name'] = $saveName;
        //执行上传操作
        $info = X('Xattach')->upload('show', $options);
        return $info;
    }
