...
class AdminAction extends AdministratorAction {
    ...
    public function userList() {
        ...
        if (!empty($_POST)) {
            $_SESSION['admin_show_user_search'] = serialize($_POST);
        } else if (isset($_GET[C('VAR_PAGE')]) || isset($_GET['orderKey'])) {
            $_POST = unserialize($_SESSION['admin_show_user_search']);
        } else {
            unset($_SESSION['admin_show_user_search']);
        }
        ...
        $map['isDel'] = 0;
        $list = M('show_user')->table("{$db_prefix}show_user AS a ")
                ->join("{$db_prefix}show_user_img AS b ON a.id=b.uid")
                ->join("{$db_prefix}show_user_flash AS c ON a.id=c.uid")
