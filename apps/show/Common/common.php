...
//根据存储路径，获取图片真实URL
function getThumb($savepath,$width=190,$height=240,$t='f') {
    $thumb = SITE_URL . "/thumb.php?w=$width&h=$height&t=$t&url=" . './data/uploads/show/'.$savepath;
    return $thumb;
}

function realityImageURL($img) {
    $imgURL = sprintf('%s/apps/show/Tpl/default/Public/show/%s', SITE_URL, $img); //默认的礼物图片地址
    if (file_exists(sprintf('./apps/show/Tpl/default/Public/show/%s', $img))) {
        return $imgURL;
    } else {//若默认里没有则返回自定义的礼物图片地址
        return sprintf('%s/data/uploads/show/%s', SITE_URL, $img);
    }
}
