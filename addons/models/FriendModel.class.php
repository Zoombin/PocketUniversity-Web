	...
	protected function _getRelatedUserFromFriend($uid, $related_uids, $limit = 20)
	{
		if ($limit <= 0)
			return array();
                ...
                $now_following =  D('Follow' ,'weibo')->getNowFollowing($uid);
//                $has_following = $now_following;
//                $has_following[] = $uid;
//                $has_following = array_merge($related_uids,$has_following);
                $map['uid'] = array('in', $now_following);
                $res = M('weibo_follow')->where($map)->field('fid')->limit($limit)->select();
                if ($res)
