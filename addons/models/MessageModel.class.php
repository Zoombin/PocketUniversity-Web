...
class MessageModel extends Model
{
     ...
     * @return array
     */
    public function getMessageByListId($list_id, $uid, $since_id = null, $max_id = null, $count = 20,$page=1) {
        ...
        $res['data']  = M('message_content')->where($where)->order('message_id DESC')->limit("$offset,$count")->findAll();
    ...
    public function getMessageByUid($uid, $mid, $since_id = null, $max_id = null, $count = 20,$page=1) {
        $res = M('message_list')->where("min_max='".$uid.'_'.$mid."' OR min_max='".$mid.'_'.$uid."'")->field('list_id')->find();
        if($res){
            return $this->getMessageByListId($res['list_id'],$mid,$since_id,$max_id,$count,$page);
        }else{
            return array();
        }
    }
     ...
     */
    public function postMessage($data, $from_uid,$tContent=true) {
        if($tContent){
        ...
        // 添加或更新私信list
        if (false == $data['list_id'] = $this->_addMessageList($data, $from_uid,$tContent)) {
            return false;
        }
        // 存储私信成员
        ...
        // 存储内容
        if (false == $this->_addMessage($data, $from_uid, $tContent)) {
            return false;
        }
        //推送
        if($from_uid=='33654'){
            $fromUser = D('User', 'home')->getUserByIdentifier($from_uid, 'uid');
            $n_extras = array('type'=>1,'uid'=>$from_uid,'uname'=>$fromUser['uname']);
            foreach($data['to'] as $to){
                service('Jpush')->sendToUid($n_extras, $to,$data['content']);
            }
        }
        return $data['list_id'];
    }
     ...
     */
    private function _addMessageList($data, $from_uid,$tContent=true)
    {
    ...
    // 添加私信message
    private function _addMessage($data, $from_uid,$tContent=true)
    {
