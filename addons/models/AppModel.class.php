...
class AppModel extends Model {
	...
	public function getOpenAppByPage($limit = 20, $field = '*', $order = 'display_order ASC') {
		return $this->where("`status`<>0 and app_name <>'".'group'."'")->field($field)->order($order)->findPage($limit);
	}
	...
	public function getAdminApp($field = '*', $order = 'display_order ASC') {
		...
		$map['status'] = array('neq', 0);
		$res = $this->where($map)->field($field)->order($order)->findAll();
                $res[] = $this->where("app_name = 'group'")->field($field)->find();
                return $res;
	}
