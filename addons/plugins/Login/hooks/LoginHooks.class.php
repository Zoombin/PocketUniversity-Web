...
class LoginHooks extends Hooks {
    ...
    private static $validLogin = array("sina" => array ("sina_wb_akey", "sina_wb_skey" ),
	...
	private static $validPublish = array("sina", "qq", "renren");
	private static $validAlias   = array('sina' => '新浪微博', 'qq' => '腾讯微博','douban'=>"豆瓣",'qzone'=>"QQ空间",'renren'=>"人人网");
	private static $validApply  = array('sina' => 'http://open.weibo.com/', 'qq' => 'http://open.t.qq.com/websites/',
	...
	public function home_index_middle_publish() {
        ...
        public function cron_sync_weibo(){
            //model('Xdata')->lput('sync_weibo',array('sina'=>3550339124915552));
            //model('Xdata')->put('sync_weibo:qq',1,true);
            //获取同步发布配置
            $config = model('AddonData')->lget('login');
            $xdata = model('Xdata');
            foreach ($config['publish'] as $value){
                //获取绑定人的信息
                $map['type'] = $value;
                $map['is_sync'] = 1;
                $loginUser = M('login')->where($map)->findAll();
                if($loginUser){
                    $since = (double)$xdata->get('sync_weibo:'.$value);
                    $this->_loadTypeLogin($value);
                    $platform = new $value();
                    foreach ($loginUser as $user) {
                        $platform->syncWeibo($user['oauth_token'],$user['uid'],$user['type_uid'],$since);
                    }
                }
