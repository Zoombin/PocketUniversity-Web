...
class VideoHooks extends AbstractWeiboTypeHooks
{
    ...
    public function _weiboTypePublish($type_data)
    {
        ...
        if(preg_match("/(youku.com|youtube.com|5show.com|ku6.com|sohu.com|mofile.com|sina.com.cn|tudou.com|yixia.com|t.cn)$/i", $parseLink['host'], $hosts)) {
    ...
    public function paramUrl()
    {
        $link = t($_POST['url']);
        $parseLink = parse_url($link);
        if(preg_match("/(youku.com|youtube.com|ku6.com|sohu.com|mofile.com|sina.com.cn|tudou.com|yixia.com|t.cn)$/i", $parseLink['host'], $hosts)) {
            $return['boolen'] = 1;
    ...
    private function _video_getflashinfo($link, $host)
    {
        ...
        elseif('yixia.com' == $host || 't.cn' == $host)
        {
            preg_match("/\?scid=(.*?)&token=/i",$content,$flashvar);
            $img[1] = 'http://paikeimg.video.sina.com.cn/stream/'.$flashvar[1].'_m.jpg';
            preg_match("/<title>(.*?)<\/title>/i",$content,$title);
            if($title[1] == '')
                $title[1] = '新浪秒拍';
        }
