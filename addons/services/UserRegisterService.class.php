...
class UserRegisterService extends Service
{
    ...
    public function register($sid, $number, $uname, $password, $is_init = 0)
    {
        // 检查站点是否关闭注册
        ...
        if($school && $school['email'] != ''){
            $email = $number.$school['email'];
        }else{
            $this->last_error = '您所在学校的账号都已导入，请用学号登录';
            return false;
        }
        ...
        $hasEmail = M('user')->where('`email`="'.$email.'"')->find();
        if ($hasEmail){
            $this->last_error = '您的账号已注册，请用学号登录';
            return false;
        }
        // 参数合法. So, continue...
