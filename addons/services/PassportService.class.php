...
class PassportService extends Service
{
    ...
    public function outLogin($num, $sid) {
        unset($_SESSION['mid']);
        unset($_SESSION['uname']);
        unset($_SESSION['zusatz']);
        unset($_SESSION['userInfo']);
        unset($_SESSION['TeacherAdmin']);
        $school = M('school')->where('pid=0 and id=' . $sid)->field('email')->find();
        if (!$school) {
            $this->_error = '获取学校信息失败';
            return false;
        }
        $email = $num . $school['email'];
        ...
        if ($user['event_level'] != 20 ||
                $user['can_event'] || $user['can_event2'] || $user['can_gift'] || $user['can_announce']
                || $user['can_print'] || $user['can_admin'] || $user['can_group']
                || $user['can_prov_event'] || $user['can_prov_news'] || $user['can_prov_work']) {
            $_SESSION['TeacherAdmin'] = $sid;
        }
        return $user;
    }
	...
	public function getLocalUser($identifier, $password = null)
	{
		...
		if (!$user) {
			$this->_error = '获取用户信息失败';
			return false;
		}else if ($password && md5($password) != $user['password'] && codePass($password) != $user['password']){
			$this->_error = '密码错误';
			return false;
		}else if ($user['is_active']==0){
			$this->_error = '用户未激活';
	 ...
	 * 检查是否登录Pu后台
	 */
	public function isLoggedPu()
	{
		return $_SESSION['PuAdmin'] == '1';
	}
	...
	public function loginPu($identifier, $password){
		if (empty($identifier) || empty($password))
			return false;

                if (!($user = $this->getLocalUser($identifier, $password))) {
			unset($_SESSION['PuAdmin']);
			return false;
		}

		// 检查是否拥有home/Puback/index权限
		if ( service('SystemPopedom')->hasPopedom($user['uid'], 'home/Puback/index', false) ) {
			$_SESSION['PuAdmin']	= 1;
			$_SESSION['mid']			= $user['uid'];
			$_SESSION['uname']			= $user['uname'];
	 ...
	 * @return boolean
	 */
	public function loginSchoolAdmin($identifier, $password='', $sid)
	{
            if (empty($identifier))
                    return false;
            ...
            // 检查是否拥有admin权限
            $isAllAdmin = M('user_group_link')->where('uid='.$user['uid'].' AND user_group_id='.C('SADMIN'))->field('uid')->find();
            if ($isAllAdmin) {
                    $_SESSION['TeacherAdmin']	= 123456;
                    $_SESSION['ThinkSNSAdmin']	= 1;
                    $_SESSION['mid']			= $user['uid'];
                    $_SESSION['uname']			= $user['uname'];
                    return true;
            } elseif($user['sid'] == $sid && ($user['event_level'] !=20 ||
                    $user['can_event'] ||$user['can_event2'] ||$user['can_gift']||$user['can_announce']
                    ||$user['can_print']||$user['can_admin']||$user['can_group']
                    ||$user['can_prov_event']||$user['can_prov_news']||$user['can_prov_work'])) {
                    $_SESSION['TeacherAdmin']	= $sid;
    ...
    public function loginCourseAdmin($identifier, $password, $sid) {
        if (empty($identifier) || empty($password))
            return false;

        if (!($user = $this->getLocalUser($identifier, $password))) {
            unset($_SESSION['CourseAdmin']);
            return false;
        }
        ...
        $isAllAdmin = M('user_group_link')->where('uid='.$user['uid'].' AND user_group_id='.C('SADMIN'))->field('uid')->find();
        if ($isAllAdmin) {
            $_SESSION['CourseAdmin'] = 123456;
            $_SESSION['mid'] = $user['uid'];
            $_SESSION['uname'] = $user['uname'];
            return true;
        } elseif ($user['sid'] == $sid) {
            $groups = M('user_group_link')->where('uid='.$user['uid'])->field('user_group_id')->findAll();
            $gids = getSubByKey($groups, 'user_group_id');
            if(in_array(C('KCADMINGID'), $gids) || in_array(C('KCGLGID'), $gids)){
                $_SESSION['CourseAdmin'] = $sid;
                $_SESSION['mid'] = $user['uid'];
                $_SESSION['uname'] = $user['uname'];
                return true;
            }else{
    ...
    public function logoutCourseAdmin() {
        // 注销session
        unset($_SESSION['mid']);
        unset($_SESSION['uname']);
        unset($_SESSION['userInfo']);
    ...
    public function logoutSchoolAdmin() {
		// 注销session
		unset($_SESSION['mid']);
		unset($_SESSION['uname']);
		unset($_SESSION['userInfo']);
...
if(in_array($uid, $uids)){
    $data = array('uid'=>$uid,'day'=>  date('Y-m-d'));
    M('tg_login')->add($data);
}
                $this->countLogin($uid);
	}
        ...
        //统计登录人数
        public function countLogin($uid){
            $dao = M('login_count');
